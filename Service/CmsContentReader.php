...
{
    private PublishedContentSerializer $publishedContentSerializer;

    public function __construct(
        private readonly AppKernelProvider $kernelProvider,
    ) {
        $this->publishedContentSerializer = new PublishedContentSerializer();
    }
    ...
    {
        $summary = $this->publishedContentSerializer->summary($content, $contentType, $locale);
    ...
    private function detail(ContentInterface $content, string $contentType, ?string $locale, bool $includeData): array
    {
        ...
        if ($publishedVersion instanceof ContentVersionInterface) {
            $detail = $this->publishedContentSerializer->detail($content, $contentType, $publishedVersion, $locale, $includeData);
        } else {
